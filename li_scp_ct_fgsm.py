...
def batch_grad(x, one_hot, i, beta, grad):
    ...
    x_neighbor_2 = 1 / 2 * input_mix_uni(x_neighbor)
    x_neighbor_4 = 1 / 4 * input_mix_uni(x_neighbor)
    x_neighbor_8 = 1 / 8 * input_mix_uni(x_neighbor)
    x_neighbor_16 = 1 / 16 * input_mix_uni(x_neighbor)
...
def look_ahead(x, one_hot, i, x_max, x_min, momentum, grad, variance):
    ...
    x_nes_2 = 1 / 2 * input_mix_uni(x_nes)
    x_nes_4 = 1 / 4 * input_mix_uni(x_nes)
    x_nes_8 = 1 / 8 * input_mix_uni(x_nes)
    x_nes_16 = 1 / 16 * input_mix_uni(x_nes)
...
def input_mix_uni(input_tensor):
    rnd = tf.random_uniform((), FLAGS.patch_size, FLAGS.image_width, dtype=tf.int32)

    start_x1 = tf.random_uniform((), 0, FLAGS.image_width - rnd, dtype=tf.int32)
    start_y1 = tf.random_uniform((), 0, FLAGS.image_height- rnd, dtype=tf.int32)
    ...
    patch1 = tf.image.crop_to_bounding_box(input_tensor, start_y1, start_x1, rnd, rnd)

    patch_pad1 = tf.image.pad_to_bounding_box(patch1, start_y2, start_x2, FLAGS.image_height, FLAGS.image_width)

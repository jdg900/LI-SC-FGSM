...
def look_ahead(x, one_hot, i, x_max, x_min, momentum, grad, variance):
    ...
    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        logits_v3, end_points_v3 = inception_v3.inception_v3(
            x_nes, num_classes=1001, is_training=False, reuse=tf.AUTO_REUSE)
    ...
    noise = momentum + (current_grad) / tf.reduce_mean(tf.abs(current_grad), [1, 2, 3], keep_dims=True)
...
def graph(x, y, i, x_max, x_min, grad):
    ...
    iter = tf.constant(0)
    _, _, _,  _, _, _, look_ahead_grad, _ = tf.while_loop(grad_finish, look_ahead, [x, y, iter, x_max, x_min, tf.zeros_like(x), tf.zeros_like(x),tf.zeros_like(x)])
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        x_adv, _, _, _, _, _ = tf.while_loop(stop, graph, [x_input, one_hot, i, x_max, x_min, grad])
